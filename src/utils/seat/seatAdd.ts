...
//初始化座位
export function initSeatDataByAdd() {
  shapes = []; //所有票图图形
...
function onKeydown({ ctrlKey, key }) {
  ...
  if (ctrlKey) {
    ...
    //回车触发事件,设置座位
    setSeatStatus(true);
    seatAddProps.setSeatData?.(shapes);
  } else if (key === 'Delete') {
    //删除触发事件,清除座位
    setSeatStatus(false);
    seatAddProps.setSeatData?.(shapes);
...
export const initSeatByAdd = function (props: SeatAddProps) {
  seatAddProps = props;
...
export const destroySeatByAdd = function () {
  // 移除监听事件
  seatCvsEventDestroy();
  // 移除座位
  shapes = [];
