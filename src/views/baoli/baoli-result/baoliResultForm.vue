<template>
  <PageWrapper title="保利返回结果">
    <CollapseContainer title="添加保利返回结果">
...
<script lang="ts">
  ...
  import { formSchema } from '/@/views/baoli/baoli-result/baoliResult.data';
  import { useI18n } from '/@/hooks/web/useI18n';
  import { createBaoliResult, findOne, updateBaoliResult } from '/@/api/baoli/baoliResult';
    ...
    setup() {
      ...
      async function handleSubmit() {
        ...
        if (unref(isUpdate)) {
          ...
          await updateBaoliResult(values);
        } else {
          await createBaoliResult(values);
        }
        await handleSuccess();
        await closeCurrent();
        router.back();
        ...
        return new Promise((resolve) => {
          ...
          createSuccessModal({
            ...
            content: unref(isUpdate)
              ? t('sys.api.updateSuccessMsg', ['保利返回结果'])
              : t('sys.api.createSuccessMsg', ['保利返回结果']),
            closable: false,
