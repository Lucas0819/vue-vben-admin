<template>
  <PageWrapper title="保利座位信息">
    <CollapseContainer title="添加保利座位信息">
...
<script lang="ts">
  ...
  import { formSchema } from '/@/views/baoli/baoli-seat/baoliSeat.data';
  import { useI18n } from '/@/hooks/web/useI18n';
  import { createBaoliSeat, findOne, updateBaoliSeat } from '/@/api/baoli/baoliSeat';
    ...
    setup() {
      ...
      async function handleSubmit() {
        ...
        if (unref(isUpdate)) {
          ...
          await updateBaoliSeat(values);
        } else {
          await createBaoliSeat(values);
        }
        await handleSuccess();
        await closeCurrent();
        router.back();
        ...
        return new Promise((resolve) => {
          ...
          createSuccessModal({
            ...
            content: unref(isUpdate)
              ? t('sys.api.updateSuccessMsg', ['保利座位信息'])
              : t('sys.api.createSuccessMsg', ['保利座位信息']),
            closable: false,
