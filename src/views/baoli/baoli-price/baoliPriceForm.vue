<template>
  <PageWrapper title="保利价格信息">
    <CollapseContainer title="添加保利价格信息">
...
<script lang="ts">
  ...
  import { formSchema } from '/@/views/baoli/baoli-price/baoliPrice.data';
    ...
    setup() {
      ...
      async function handleSubmit() {
        ...
        if (unref(isUpdate)) {
          ...
          await updateBaoliPrice(values);
        } else {
          await createBaoliPrice(values);
        }
        await handleSuccess();
        await closeCurrent();
        router.back();
        ...
        return new Promise((resolve) => {
          ...
          createSuccessModal({
            ...
            content: unref(isUpdate)
              ? t('sys.api.updateSuccessMsg', ['保利价格信息'])
              : t('sys.api.createSuccessMsg', ['保利价格信息']),
            closable: false,
