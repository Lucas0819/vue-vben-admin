<template>
  <PageWrapper title="折扣管理">
    <CollapseContainer title="添加折扣管理">
...
<script lang="ts">
  ...
  import { formSchema } from '/@/views/bis/bis-vipcard/bisVipcard.data';
  import { useI18n } from '/@/hooks/web/useI18n';
  import { createBisVipcard, findOne, updateBisVipcard } from '/@/api/bis/bisVipcard';
    ...
    setup() {
      ...
      async function handleSubmit() {
        ...
        if (unref(isUpdate)) {
          ...
          await updateBisVipcard(values);
        } else {
          await createBisVipcard(values);
        }
        await handleSuccess();
        await closeCurrent();
        router.back();
        ...
        return new Promise((resolve) => {
          ...
          createSuccessModal({
            ...
            content: unref(isUpdate)
              ? t('sys.api.updateSuccessMsg', ['折扣管理'])
              : t('sys.api.createSuccessMsg', ['折扣管理']),
            closable: false,
